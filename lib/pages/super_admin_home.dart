...
class SuperAdminHomePage extends StatefulWidget {
  final String? empresaIdOverride;
  final bool isSubView;
  const SuperAdminHomePage({
    super.key,
    this.empresaIdOverride,
...
class _SuperAdminHomePageState extends State<SuperAdminHomePage> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      if (widget.empresaIdOverride != null) {
        _miEmpresaId = widget.empresaIdOverride;
      } else {
      ...
      if (_miEmpresaId != null) {
        final empresaRes = await supabase
            .from('empresas')
            .select()
            .eq('id', _miEmpresaId!)
            .single();
        final sucursalesRes = await supabase
            .from('sucursales')
            .select()
  ...
  Future<void> _crearSucursal() async {
    ...
    if (disponibles <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "No tienes rutas disponibles para asignar a una nueva sucursal.",
          ),
        ),
      );
      return;
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (r > disponibles) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text("Excedes tu cuota disponible"),
                    ),
                  );
                  return;
                }
  ...
  Widget _buildMembershipCard(bool isDark) {
    ...
    Color estadoColor = Colors.greenAccent;
    if (!isActive) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // ── Header: icono + nombre + status dot ──
          Row(
            children: [
              Container(
                padding: const EdgeInsets.all(10),
                ...
                child: Icon(Icons.business_rounded, color: primary, size: 22),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
          ...
          // ── Info chips: vencimiento y teléfono ──
          _membershipInfoChip(
          ...
          if (telefono != null && telefono.isNotEmpty) ...[
            ...
            _membershipInfoChip(Icons.phone_rounded, telefono, isDark, primary),
          ],
        ],
      ),
    );
