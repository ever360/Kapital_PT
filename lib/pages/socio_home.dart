...
class _SocioHomePageState extends State<SocioHomePage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_miEmpresaId != null) {
        ...
        final todasRutas = await supabase
            .from('rutas')
            .select('id')
  ...
  Future<void> _crearRuta() async {
    ...
    if (_rutasHermanasTotales >= rutasMaximas) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Límite de Rutas alcanzado para tu empresa. Contacta a tu Administrador.",
          ),
  ...
  Future<void> _verOperariosDeLaRuta(Map<String, dynamic> ruta) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error al cargar operarios: $e')));
    }
  }

  void _mostrarPanelGestionRuta(
                  ...
                  children: [
                    ...
                    ...asignados.map((a) {
                      return ListTile(
                        ...
                        subtitle: Text(
                          a['profiles']['rol'].toString().toUpperCase(),
                          style: TextStyle(
                            color: isDark ? Colors.white54 : Colors.black54,
                            fontSize: 10,
                          ),
                    ...
                    if (catalogo.isEmpty)
                      Text(
                        'No hay cobradores activos en la empresa.',
                        style: TextStyle(
                          color: isDark ? Colors.white54 : Colors.black54,
                        ),
                      ),
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (ctx, idx) {
                          ...
                          final yaAsignado = asignados.any(
                            (asig) => asig['perfil_id'] == candidato['id'],
                          );
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                              candidato['rol'].toString().toUpperCase(),
                              style: TextStyle(
                                color: isDark ? Colors.white54 : Colors.black54,
                                fontSize: 10,
                              ),
                  ...
                  children: [
                    ...
                    if (_misRutas.isEmpty)
                      const Padding(
                        ...
                        child: Center(
                          child: Text(
                            "Aún no tienes rutas. ¡Crea una para empezar!",
                          ),
                        ),
                      )
                    else
